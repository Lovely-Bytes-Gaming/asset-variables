...
{
    private const string script_path = "Assets/ScriptableVariables/Scripts/ScriptableVariables/AutoGenerated/Enums/";
...
{{
    ...
    private void OnGUI()
    {
        ...
        if(tmp != numValues)
        {
            ...
            {
                Debug.LogWarning("init array");
                lastKnownValues = new string[tmp];
            }
            else
            {
                Debug.LogWarning("array existing");
        ...
        {
            // TODO validate enum name
            //Debug.Log(IsScriptNameValid(nameStr));
            ...
            try
            {
                StreamWriter writer = new StreamWriter(script_path + string.Format(script_name, nameStr), false);
                writer.Write(scriptStr);
                writer.Close();

                writer = new StreamWriter(editor_path + string.Format(editor_name, nameStr), false);
                writer.Write(editorStr);
                writer.Close();
    ...
    {
        // should start with a letter and can only contain numbers, letters and undercores
